...
template<typename T> Vector2<T> operator+(Vector2<T> const& v1, Vector2<T> const& v2){ return {v1.x + v2.x, v1.y +v2.y, v1.z+v2.z, v1.w+v2.w}; }
template<typename T> Vector2<T> operator-(Vector2<T> const& v1, Vector2<T> const& v2){ return {v1.x - v2.x, v1.y - v2.y, v1.z-v2.z, v1.w-v2.w}; }
template<typename T> Vector2<T> operator*(T const& scl, Vector2<T> const& v){ return {scl*v.x, scl*v.y, scl*v.z, scl*v.w}; }
template<typename T> Vector2<T> operator*(Vector2<T> const& v, T const& scl){ return {v.x*scl, v.y*scl, v.z*scl, v.w*scl}; }
template<typename T> Vector2<T> operator/(Vector2<T> const& v, T const& scl){ return {v.x/scl, v.y/scl, v.z/scl, v.w/scl}; }
