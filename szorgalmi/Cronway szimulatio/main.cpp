...
struct Table2D
{
	...
	void parallel_fill2_v(F&& f)
	{
        ...
        {
            if(t == n_threads-1){ ikhi = h; }
            fs[t] = std::async(std::launch::async, [this, f](int lo, int khi)
            {
                
                for(int i=0; i<w; ++i)
                {
                    for(int j=lo; j<khi; ++j)
                    {
                        (*this)(i, j) = f(i, j);
                    }
                }
    ...
    template<typename F>
	void parallel_fill2_h(F&& f)
	{
        static const int n_threads = 4;

        if(w < 100){ fill2(f); return; }
        ...
        for(int t=0; t<n_threads; ++t)
        {
            if(t == n_threads-1){ iright = h; }
            fs[t] = std::async(std::launch::async, [this, f](int r, int l)
            {
...
struct App
{
	...
	int enterApp()
	{
		...
		wnd.idleHandler([&]
		{
			...
			table[1 - idx].parallel_fill2_h([&t = table[idx]](int x, int y)->char
			{
				auto w = t.w;
				auto h = t.h;
				if(w < 1 || h < 1){ return 0; }
		...
		wnd.renderHandler( [&](SoftwareRenderer& r)
		{
            ...
            if(frame_count == 200)
            {
                std::cout << "Average time of step: " << frame_time / frame_count << std::endl;
                frame_time = 0.0;
                frame_count = 0;
            }
		});

		bool res = wnd.open(L"Conway parallel simulation", {42, 64}, {640, 480}, true, [&]{ return true; });
		return res ? 0 : -1;
	}
};
